...
def apartat_b():
    ...
    for partition in partitions:
        ...
        model = LogisticRegression(
            C=2.0, fit_intercept=True, penalty='l2', tol=0.001)
        model.fit(X_train, y_train)
        print(
            f'Logistic Regression Score with part {partition}: {model.score(X_test, y_test)}\n')

        model = svm.SVC(C=10.0, kernel='rbf', probability=True, random_state=0)
        model.fit(X_train, y_train)
        print(
            f'SVC Score with part {partition}: {model.score(X_test, y_test)}\n')

        model = KNeighborsClassifier()
        model.fit(X_train, y_train)
        print(
            f'KNN Score with part {partition}: {model.score(X_test, y_test)}\n')
